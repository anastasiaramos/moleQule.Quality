...
{
    public partial class AreaMngForm : EntityLMngSkinForm
    {

        #region Attributes & Properties
		
        public const string ID = "AreaMngForm";
        public static Type Type { get { return typeof(AreaMngForm); } }
        public override Type EntityType { get { return typeof(Area); } }

		protected override int BarSteps { get { return base.BarSteps + 3; } }

        protected Area _entity;

        private new SortedBindingList<AreaInfo> _filter_results = null;
        private new SortedBindingList<AreaInfo> _sorted_list = null;
        private new SortedBindingList<AreaInfo> _search_results = null;

        /// <summary>
        ///  Lista de objetos de sólo lectura
        /// </summary>
        internal new AreaList List
        {
            get { return _item_list as AreaList; }
            set { _item_list = value; _sorted_list = (value as AreaList).GetSortedList(); }
        }
        internal new SortedBindingList<AreaInfo> SortedList { get { return _sorted_list; } }
        internal new AreaList FilteredList { get { return AreaList.GetList(_filter_results); } }
        internal SortedBindingList<AreaInfo> CurrentList { get { return (Datos.List as SortedBindingList<AreaInfo>); } }

		/// <summary>
		/// Devuelve el OID del objeto activo seleccionado de la tabla
		/// </summary>
		/// <returns></returns>
		public override long ActiveOID { get { return Datos.Current != null ? ((AreaInfo)Datos.Current).Oid : -1; } }

        /// <summary>
        /// Devuelve el objeto activo seleccionado de la tabla
        /// </summary>
        /// <returns></returns>
        public AreaInfo ActiveItem { get { return (Datos.Current != null) ? (AreaInfo)Datos.Current : null; } }
		
        public override long ActiveFoundOID { get { return (DatosSearch.Current != null) ? ((AreaInfo)(DatosSearch.Current)).Oid : -1; } }
		...
		public AreaMngForm(bool isModal)
			: this(isModal, null) {}

        public AreaMngForm(Form Parent)
            : base(false, Parent) { }
		
		public AreaMngForm(bool isModal, Form Parent)
			: base(isModal, Parent)
		{
            InitializeComponent();
		...
		{
			Tabla.Visible = Area.CanGetObject();
            Add_Button.Enabled = Area.CanAddObject();
            Edit_Button.Enabled = Area.CanEditObject();
            Delete_Button.Enabled = Area.CanDeleteObject();
            Print_Button.Enabled = Area.CanGetObject();
            View_Button.Enabled = Area.CanGetObject();
		}
        ...
        protected void SetMainList(SortedBindingList<AreaInfo> list, bool order)
        {
        ...
        public override void OpenAddForm()
        {
            try
            {
                AreaAddForm form = new AreaAddForm();
                AddForm(form);
                _entity = form.Entity;
            }
		...
		public override void OpenViewForm()
		{
			...
			try
			{
				AddForm(new AreaViewForm(ActiveOID));
			}
			catch (Csla.DataPortalException ex)
			{
				MessageBox.Show(ex.BusinessException.ToString(),
        ...
        {             
            try
            {
                AreaEditForm form = new AreaEditForm(ActiveOID);
                if (form.Entity != null)
                {
                    AddForm(form);
                    _entity = form.Entity;
		...
		public override void DuplicateObject(long oid) 
		{
			try
			{
                Area old = Area.Get(oid);
                Area dup = old.CloneAsNew();
                old.CloseSession();
				
                AddForm(new AreaAddForm(dup));
			}
		...
		{
			/*AreaReportMng reportMng = new AreaReportMng(AppContext.ActiveSchema);
			
			AreaListRpt report = reportMng.GetListReport(list);
