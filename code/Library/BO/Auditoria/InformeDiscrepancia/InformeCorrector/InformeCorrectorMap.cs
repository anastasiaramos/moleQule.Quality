...
{
	[Serializable()]
	public class InformeCorrectorMap : ClassMapping<InformeCorrectorRecord>
	{	
		public InformeCorrectorMap()
		{
			Table("`InformeCorrector`");
			Lazy(true);	
			
			Id(x => x.Oid, map => { map.Generator(Generators.Sequence, gmap => gmap.Params(new { sequence = "`InformeCorrector_OID_seq`" })); map.Column("`OID`"); });
